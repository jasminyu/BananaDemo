...
function (angular, app, _, kbn, moment) {
	...
	module.controller('table', function($rootScope, $scope, fields, querySrv, dashboard, filterSrv) {
		...
		$scope.$watch('panel.offset',function(){
			var start = $scope.panel.offset;
			var end = $scope.panel.offset + $scope.panel.size;
	if(!$scope.data){
		return;  
		}
		if (start < $scope.data.length) {
				$scope.data.slice(start,end);
			}
		else
		{
				$scope.get_next_Page_data($scope.data,start,end);
		}
		});
	...
	$scope.get_next_Page_data = function(data,start,end)
	{
		var request = $scope.panel_request;
		$scope.panelMeta.loading = true;
		//if exist cursor， add course to query paramter
		var query_cursor = "";
		if($scope.panel.nextCursorMark && $scope.panel.nextCursorMark.length>0)
			{
		query_cursor = "&nextCursorMark="+$scope.panel.nextCursorMark; 
			}

			request = request.setQuery($scope.history_url+query_cursor);
			
			var response = request.doSearch();

			response.then(function(results) {
		 $scope.panelMeta.loading = false;
		 $scope.panel.nextCursorMark = escape(results.response.nextCursorMark);
		...
		$scope.get_data = function(segment,query_id,isUpdateSortState) {
			...
			if (query_size !== undefined && query_size !== 0) {
					var _pages = 10;
					if($scope.panel.pages !== undefined && $scope.panel.pages !== 0)
					{
						if($scope.panel.pages <10)
						{
							_pages = $scope.panel.pages;
						}							
					}
					rows_limit =  '&rows=' + $scope.panel.size * _pages;
					total_limit = '&total=' + query_size;
						$scope.panel.table_total = query_size;
			} else { // default
					rows_limit = '&rows=25';
					//µ±ÓÃ»§Î´ÊäÈë»òÊäÈëÓÐÎó£¬Ä¬ÈÏ×îÖµÎª25*10
					total_limit = '&total=250';
						$scope.panel.table_total = 250;
			}
			...
			$scope.panel.queries.query = $scope.panel.queries.basic_query + wt_json + rows_limit + total_limit;
			var sort_queries_query = $scope.panel.queries.basic_query + wt_json + sort_rows_limit + total_limit;
			// Set the additional custom query
			if ($scope.panel.queries.custom != null) {
				request = request.setQuery(sort_rows_limit?sort_queries_query:$scope.panel.queries.query + $scope.panel.queries.custom);
		$scope.history_url = $scope.panel.queries.query + $scope.panel.queries.custom;
			} else {
				request = request.setQuery(sort_rows_limit?sort_queries_query:$scope.panel.queries.query);
		$scope.history_url = $scope.panel.queries.query;
			}
			...
			results.then(function(results) {
				$scope.panelMeta.loading = false;
			$scope.panel.nums = results.response.nums;
		$scope.panel.nextCursorMark = escape(results.response.nextCursorMark);
		...
		$scope.exportfile = function(filetype) {
			...
			var exportQuery = $scope.panel.queries.basic_query + '&wt=' + filetype + omitHeader + rows_limit + fl + '&op=exportfile';
			var request = $scope.panel_request;
