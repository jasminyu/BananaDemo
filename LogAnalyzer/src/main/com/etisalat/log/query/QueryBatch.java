...
public class CursorRspProcessor implements RspProcess {
    ...
    private Set<String> involvedCollections = null;
    ...
    public CursorRspProcessor(QueryCondition queryCondition, Cursor cursor, long realReturnNum,
        ...
        setMaxCollectionAndInvolvedCollections();
        setMaxCollShardId();
    }
    ...
    private void setMaxCollectionAndInvolvedCollections(){
        ...
        this.involvedCollections = new HashSet<String>();
        this.involvedCollections.addAll(collections);
    }
    ...
    private String writeJsonFiles(Set<Map.Entry<String, List<String>>> entrySet) throws IOException {
        ...
        for (Map.Entry<String, List<String>> entry : entrySet) {
            ...
            for (String jsonObjStr : jsonObjects) {
                ...
                //add by xfx
                rspJsonList.add(SolrUtils.convertJsonObject(JsonUtil.fromJson(jsonObjStr, JsonObject.class)));
                //add by xfx
        ...
        try {
            ...
            for (Map.Entry<String, List<String>> entry : entrySet) {
                ...
                for (String jsonObjStr : jsonObjects) {
                    ...
                    docNum++;
                    //add by xfx
                    xmlWriter.writeXmlJson(SolrUtils.convertJsonObject(JsonUtil.fromJson(jsonObjStr, JsonObject.class)));
                    //add by xfx
                    if (docNum % singleFileSize == 0) {
                        xmlWriter.writeEnd();
                        xmlWriter.flush();
        ...
        for (Map.Entry<String, List<String>> entry : entrySet) {
            ...
            for (String jsonObjStr : jsonObjects) {
                ...
                if (LogConfFactory.keepCsvHeader && csvHeader == null) {
                    //add by xfx
                	csvHeader = SolrUtils.getCsvHeader(SolrUtils.convertJsonObject(jsonObj));
                	//add by xfx
                ...
                for (Map.Entry<String, JsonElement> entryElement : set) {
                	//add by xfx
                	String val = entryElement.getValue().getAsString();
                	if(val.contains(",")){
                		val = "\"" + val + "\"";
                	}
                    if (first) {
    ...
    private String writeJsonFiles(List<JsonObject> jsonObjects) throws IOException {
        ...
        for (JsonObject jsonObj : jsonObjects) {
            ...
            //add by xfx
            rspJsonList.add(SolrUtils.convertJsonObject(jsonObj));
            //add by xfx
            if (docNum % singleFileSize == 0) {
                rspJson.add("docs", rspJsonList);
                write(getFileName(fileIdx++), JsonUtil.toJson(resultObj));
    ...
    private String writeXmlFiles(List<JsonObject> jsonObjects) throws IOException {
        ...
        try {
            ...
            for (JsonObject jsonObj : jsonObjects) {
                ...
                docNum++;
                //add by xfx
                xmlWriter.writeXmlJson(SolrUtils.convertJsonObject(jsonObj));
                //add by xfx
                if (docNum % singleFileSize == 0) {
                    xmlWriter.writeEnd();
                    xmlWriter.flush();
    ...
    public String processDisplay() throws LogQueryException {
        ...
        while (true) {
            ...
            for (; idx < jsonListSize; idx++) {
                ...
                //add by xfx
                resultJsonObjArray.add(SolrUtils.convertJsonObject(JsonUtil.fromJson(jsonObjectList.get(idx),  JsonObject.class)));
                //add by xfx
                cnt++;
            }
    ...
    private String getNextCollection(String collection) throws LogQueryException {
    	String coll = null;
    	while(true) {
    		coll = SolrUtils.getNextCollection(collection, maxCollection);
    		if (coll.equals(maxCollection)) {
    			return coll;
    		}
    		if(!involvedCollections.contains(coll)) {
    			collection = coll;
    			continue;
    		}
    		return coll;
